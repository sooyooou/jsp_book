...
public class BookDBBean2 {

	private static BookDBBean2 instance = new BookDBBean2();

	// .jsp페이지에서 DB연동빈인 BoardDBBean클래스의 메소드에 접근시 필요
	public static BookDBBean2 getInstance() {
		return instance;
	}
	...
	public List<BookDataBean> getArticles(String book, int start, int end) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			pstmt = conn.prepareStatement("select * from book where booktype = ? order by num asc limit ?,?;");
	...
	public String image(String book, int num) {
		...
		try {
			...
			pstmt = conn.prepareStatement("select image from book where booktype = ? and num = ? order by num asc");
			pstmt.setString(1, book);
			pstmt.setInt(2, num);
			rs = pstmt.executeQuery();
